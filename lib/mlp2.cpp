...
namespace abed {
    ...
    MLP2::MLP2 (unsigned int d, unsigned int l, const vector<unsigned int> &hl, double lr, double mom) 
    ...
    double MLP2::train (const StaticDataSet& sds) {
    ...
    void MLP2::classify (StaticDataSet& sds) const {
        for (unsigned int n = 0; n < sds.get_size(); n++) {
    ...
    unsigned int MLP2::predict (const StaticDataPoint& x) const {
        compute_outputs(x);
    ...
    void MLP2::compute_outputs (const StaticDataPoint& x) const {
